...
//where
    ...
    */
    ...
    JCExpression term() {
        ...
        //"+="到">>>="在Token.java中的定义顺序正好对应"PLUSEQ.compareTo(S.token()) <= 0 && S.token().compareTo(GTGTGTEQ) <= 0"
        //PLUSEQ.compareTo(S.token()) <= 0表示PLUSEQ的值(ordinal)<=S.token()
        //compareTo()方法在java.lang.Enum<E>定义
    ...
    JCExpression term2() {
		...
		//当前运算符的优先级>=“||”运算符的优先级时，才调用term2Res
...
//todo: skip to anchor on error(?)
    ...
    loop:
        ...
        if (pos != Position.NOPOS)
            storeEnd(mods, S.prevEndPos());//storeEnd()只是一个空方法
    ...
    JCVariableDecl variableDeclaratorId(JCModifiers mods, JCExpression type) {
        ...
        if ((mods.flags & Flags.VARARGS) == 0)
            type = bracketsOpt(type);//mothodName(N[] n[],S s)这种语法也不会报错
        //形参没有初始化部分，所以VarDef方法的第4个参数为null
    ...
    public JCTree.JCCompilationUnit compilationUnit() {
        ...
        String dc = S.docComment();//当前token对应的注释,这个版本的javac都返回null
        JCModifiers mods = null;//对应文法中的{ "@" Annotation }
     ...
     *      |ClassOrInterfaceOrEnumDeclaration
     */
     //ClassOrInterfaceOrEnumDeclaration与( Type Ident......)是并列的,不是被包含的关系
     //请看“18.1. The Grammar of the Java Programming Language”
            ...
            //语句块(包括static语句块(STATIC关键字在modifiersOpt()中已分析过))
                ...
                if (S.token() == LPAREN && !isInterface && type.tag == JCTree.IDENT) {
                    if (isInterface || name != className)
                        log.error(pos, "invalid.meth.decl.ret.type.req");//方法声明无效；需要返回类型
